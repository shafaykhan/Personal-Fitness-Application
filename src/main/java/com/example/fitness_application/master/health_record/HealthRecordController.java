...
public class HealthRecordController {
  ...
  @PostMapping
  public ResponseEntity<HealthRecordDTO> saveOrUpdate(@Valid @RequestBody HealthRecordDTO payloadDTO) {
    return ResponseEntity.ok(service.saveOrUpdate(payloadDTO));
  }

  @GetMapping("/by-user/{userId}")
  public ResponseEntity<List<HealthRecordDTO>> findAllByUserId(@PathVariable Long userId) {
    return ResponseEntity.ok(service.findAllByUserId(userId));
  }

  @GetMapping("/{id}")
  public ResponseEntity<HealthRecordDTO> findById(@PathVariable Long id) {
